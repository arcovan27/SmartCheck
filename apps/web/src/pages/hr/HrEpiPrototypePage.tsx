...
import { HrEpiHistorySection } from "./HrEpiHistoryPage";
...
type Movement = { id: string; movementType: string; quantity: number; date: string; movementReason?: string | null; unitCostSnapshot?: string | null; isEstimatedCost: boolean; departmentSnapshot?: string | null; employee: EmployeeOption; epi: { id: string; name: string; ca: string; unit: string }; unit?: { id: string; name: string } | null; responsibleUser?: { email?: string; employee?: { name?: string } | null } | null };
...
export function HrEpiPrototypePage() {
  ...
  const createMovement = useMutation({
    ...
    onSuccess: async () => {
      ...
      await Promise.all([queryClient.invalidateQueries({ queryKey: ["hr-epis"] }), queryClient.invalidateQueries({ queryKey: ["hr-epi-movements"] }), queryClient.invalidateQueries({ queryKey: ["hr-epi-dashboard"] }), queryClient.invalidateQueries({ queryKey: ["hr-epi-history"] })]);
    }
  });
  ...
  return (
    <div className="space-y-5">
      <HrSectionTabs />
      <HrPageHeader eyebrow="Recursos Humanos · EPI" title="EPI" description="Indicadores, estoque, movimentações e histórico em uma única página, dentro do escopo autorizado." action={canManage ? <button className="rounded-xl bg-brand-400 px-4 py-2.5 text-sm font-extrabold text-slate-950 hover:bg-brand-300" onClick={() => setShowMovement(true)}>Nova movimentação</button> : undefined} />

      <section className="card p-5">
        <SectionHeading title="Indicadores e custos" description="Indicadores calculados diretamente das movimentações do período" action={<div className="flex gap-2"><input className="input" aria-label="Início do período" type="date" value={period.startDate} onChange={(event) => setPeriod({ ...period, startDate: event.target.value })} /><input className="input" aria-label="Fim do período" type="date" value={period.endDate} onChange={(event) => setPeriod({ ...period, endDate: event.target.value })} /></div>} />
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><MetricCard label="Total de EPIs" value={String(data?.summary.registered ?? "—")} detail="Itens cadastrados no escopo atual" /><MetricCard label="EPIs entregues" value={String(dashboardQuery.data?.totalDelivered ?? "—")} detail="Entregas e substituições no período" tone="success" /><MetricCard label="Movimentações" value={String(dashboardQuery.data?.movementCount ?? "—")} detail="Todos os tipos registrados" tone="info" /><MetricCard label="Funcionários com entregas" value={String(dashboardQuery.data?.employeesRelated ?? "—")} detail="Funcionários distintos nas entregas" />{dashboardQuery.data?.canViewCosts ? <MetricCard label="Custo das entregas" value={dashboardQuery.data.totalCost === null ? "—" : currency.format(dashboardQuery.data.totalCost)} detail="Valor histórico das movimentações" tone="warning" /> : null}</div>
      ...
      {canManage ? <section className="card border-amber-200 bg-amber-50 p-5"><SectionHeading title="Entrega de EPI" description="Ação preservada para reativação quando o leitor biométrico estiver disponível." />{deliveryFormEnabled ? <Link className="btn-primary inline-flex" to="/recursos-humanos/epi/ficha-entrega">Realizar entrega de EPI</Link> : <button type="button" aria-disabled="true" className="cursor-not-allowed rounded-xl border border-amber-300 bg-white px-4 py-3 text-left text-sm font-extrabold text-amber-950 opacity-80" onClick={() => setDeliveryMessage("A entrega de EPI está temporariamente indisponível.")}><span>Realizar entrega de EPI</span><span className="ml-3 rounded-full bg-amber-200 px-2 py-1 text-[10px] uppercase text-amber-900">Temporariamente indisponível</span></button>}{deliveryMessage ? <p className="mt-3 text-sm font-bold text-amber-900" role="status">{deliveryMessage}</p> : null}</section> : null}
      ...
      <section id="movimentacoes" className="card scroll-mt-5 p-5">
        <SectionHeading title="Movimentações recentes" description="Últimos lançamentos do escopo autorizado; custos usam o valor preservado na movimentação." />
        {movementQuery.isLoading ? <LoadingState /> : movementQuery.data?.items.length ? (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1050px] text-left text-sm">
              <thead><tr className="border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500"><th className="px-3 py-3">Data</th><th className="px-3 py-3">EPI</th><th className="px-3 py-3">Tipo</th><th className="px-3 py-3">Quantidade</th><th className="px-3 py-3">Funcionário</th><th className="px-3 py-3">Setor / unidade</th>{data?.canViewCosts ? <><th className="px-3 py-3">Custo unitário</th><th className="px-3 py-3">Custo total</th></> : null}<th className="px-3 py-3">Responsável</th><th className="px-3 py-3">Situação</th></tr></thead>
              <tbody>{movementQuery.data.items.map((movement) => {
                const unitCost = movement.unitCostSnapshot === null || movement.unitCostSnapshot === undefined ? null : Number(movement.unitCostSnapshot);
                return <tr key={movement.id} className="border-b border-slate-100 last:border-0"><td className="whitespace-nowrap px-3 py-4">{formatBrazilDateTime(movement.date)}</td><td className="px-3 py-4 font-bold">{movement.epi.name}</td><td className="px-3 py-4"><StatusBadge tone={movement.movementType === "DEVOLUCAO" ? "info" : movement.movementType === "SUBSTITUICAO" ? "warning" : "success"}>{movement.movementType}</StatusBadge></td><td className="px-3 py-4">{movement.quantity} {movement.epi.unit}</td><td className="px-3 py-4">{movement.employee.name}</td><td className="px-3 py-4">{movement.departmentSnapshot || movement.unit?.name || "—"}</td>{data?.canViewCosts ? <><td className="px-3 py-4">{unitCost === null ? "—" : currency.format(unitCost)}{movement.isEstimatedCost ? " (estimado)" : ""}</td><td className="px-3 py-4 font-bold">{unitCost === null ? "—" : currency.format(unitCost * movement.quantity)}</td></> : null}<td className="px-3 py-4">{movement.responsibleUser?.employee?.name ?? movement.responsibleUser?.email ?? "—"}</td><td className="px-3 py-4"><StatusBadge tone="success">Registrada</StatusBadge></td></tr>;
              })}</tbody>
            </table>
          </div>
