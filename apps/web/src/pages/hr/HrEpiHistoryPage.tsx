...
import { SectionHeading, StatusBadge } from "./HrPrototypeComponents";
...
export function HrEpiHistorySection() {
  ...
  return (
    <section id="historico" className="card scroll-mt-5 p-5">
      <SectionHeading title="Histórico" description="Consulta completa das movimentações, com custos preservados na data do lançamento e registros mais recentes primeiro." />
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
        ...
        <select className="select" aria-label="Todos os tipos" value={filters.movementType} onChange={(event) => setFilter("movementType", event.target.value)}>
          ...
          <table className="w-full min-w-[1100px] text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500">
                <th className="px-3 py-3">Data</th><th className="px-3 py-3">Funcionário</th><th className="px-3 py-3">EPI</th><th className="px-3 py-3">Movimentação</th><th className="px-3 py-3">Quantidade</th>
                {data?.canViewCosts ? <><th className="px-3 py-3">Valor unitário</th><th className="px-3 py-3">Valor total</th></> : null}
                <th className="px-3 py-3">Responsável</th><th className="px-3 py-3">Observações</th><th className="px-3 py-3">Situação</th>
              </tr>
            </thead>
            <tbody>{data?.items.map((movement) => {
              ...
              return (
                <tr key={movement.id} className="border-b border-slate-100 last:border-0">
                  <td className="whitespace-nowrap px-3 py-4">{formatBrazilDateTime(movement.date)}</td>
                  <td className="px-3 py-4"><p className="font-bold">{movement.employee.name}</p><p className="text-xs text-slate-500">{movement.departmentSnapshot || movement.unit?.name || "—"}</p></td>
                  <td className="px-3 py-4"><p className="font-bold">{movement.epi.name}</p><p className="text-xs text-slate-500">CA {movement.epi.ca || "—"}</p></td>
                  <td className="px-3 py-4">{movement.movementType}</td>
                  <td className="px-3 py-4">{movement.quantity} {movement.epi.unit}</td>
                  {data.canViewCosts ? <><td className="px-3 py-4">{unitCost === null ? "—" : currency.format(unitCost)}{movement.isEstimatedCost ? " (estimado)" : ""}</td><td className="px-3 py-4 font-bold">{unitCost === null ? "—" : currency.format(unitCost * movement.quantity)}</td></> : null}
                  <td className="px-3 py-4">{movement.responsibleUser?.employee?.name ?? movement.responsibleUser?.email ?? "—"}</td>
                  <td className="max-w-xs px-3 py-4 text-slate-600">{movement.movementReason || movement.notes || "—"}</td>
                  <td className="px-3 py-4"><StatusBadge tone="success">Registrada</StatusBadge></td>
                </tr>
              );
            })}</tbody>
          </table>
      ...
      <div className="mt-4 flex items-center justify-between text-sm">
        <span className="text-slate-500">{data?.total ?? 0} registro(s)</span>
        <div className="flex items-center gap-2"><button className="btn-secondary" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Anterior</button><span className="px-2 font-bold">{page} / {pageCount}</span><button className="btn-secondary" disabled={page >= pageCount} onClick={() => setPage((value) => value + 1)}>Próxima</button></div>
      </div>
    </section>
  );
}
